...
class MBart50Finetuner:
    ...
    def load_datasets(self):
        ...
        if self.id is not None:
            ...
            self.train_dataset = load_dataset(
                ...
                split=f"train{training_parts[self.id]}")
            self.test_dataset = load_dataset("csv",
                                             data_files=data_files,
                                             split="test")
            self.val_dataset = load_dataset("csv",
                                            data_files=data_files,
                                            split="val[:20000]")
        else:
            self.train_dataset = load_dataset("csv",
                                              data_files=data_files,
                                              split="train")
            self.test_dataset = load_dataset("csv",
                                             data_files=data_files,
    ...
    def finetune(self):
        ...
        preprocessor = TextPreprocessor(self.tokenizer,
                                        self.max_len,
                                        name="mbart50")
        tokenized_train_dataset = preprocessor.preprocess_dataset(
            self.train_dataset)
        tokenized_eval_dataset = preprocessor.preprocess_dataset(
            self.val_dataset)
